#!/usr/bin/env python3
"""
snow2.py - Implementação do SNOW 2.0 em Python
"""
...
class Snow2Cipher:
    ...
    def __init__(self, key):
        ...
        if key_len not in (16, 32):
            raise ValueError("Invalid key size: must be 16 or 32 bytes")
        
        self.key = key
        self.keysize = key_len * 8
        self.lfsr = [0] * 16
        self.r1 = 0
        self.r2 = 0
    
    def _byte_n(self, n, w):
        """Extrai o n-ésimo byte de w (0 = LSB, 3 = MSB)"""
        return (w >> (8 * n)) & 0xff
    
    def _a_mul(self, w):
        """a_mul do C"""
        return ((w << 8) ^ self.snow_alpha_mul[w >> 24]) & 0xFFFFFFFF
    
    def _ainv_mul(self, w):
        """ainv_mul do C"""
        return ((w >> 8) ^ self.snow_alphainv_mul[w & 0xff]) & 0xFFFFFFFF
    
    def load_key(self, IV0, IV1, IV2, IV3):
        """Carrega a chave e IV"""
        if self.keysize == 128:
            self.lfsr[15] = (self.key[0] << 24) | (self.key[1] << 16) | (self.key[2] << 8) | self.key[3]
            self.lfsr[14] = (self.key[4] << 24) | (self.key[5] << 16) | (self.key[6] << 8) | self.key[7]
            self.lfsr[13] = (self.key[8] << 24) | (self.key[9] << 16) | (self.key[10] << 8) | self.key[11]
            self.lfsr[12] = (self.key[12] << 24) | (self.key[13] << 16) | (self.key[14] << 8) | self.key[15]
            
            self.lfsr[11] = ~self.lfsr[15] & 0xFFFFFFFF
            self.lfsr[10] = ~self.lfsr[14] & 0xFFFFFFFF
            self.lfsr[9] = ~self.lfsr[13] & 0xFFFFFFFF
            self.lfsr[8] = ~self.lfsr[12] & 0xFFFFFFFF
            self.lfsr[7] = self.lfsr[15]
            self.lfsr[6] = self.lfsr[14]
            self.lfsr[5] = self.lfsr[13]
            self.lfsr[4] = self.lfsr[12]
            self.lfsr[3] = ~self.lfsr[15] & 0xFFFFFFFF
            self.lfsr[2] = ~self.lfsr[14] & 0xFFFFFFFF
            self.lfsr[1] = ~self.lfsr[13] & 0xFFFFFFFF
            self.lfsr[0] = ~self.lfsr[12] & 0xFFFFFFFF
        else:  # 256 bits
            for i in range(8):
                self.lfsr[15 - i] = (self.key[4*i] << 24) | (self.key[4*i+1] << 16) | (self.key[4*i+2] << 8) | self.key[4*i+3]
            
            self.lfsr[7] = ~self.lfsr[15] & 0xFFFFFFFF
            self.lfsr[6] = ~self.lfsr[14] & 0xFFFFFFFF
            self.lfsr[5] = ~self.lfsr[13] & 0xFFFFFFFF
            self.lfsr[4] = ~self.lfsr[12] & 0xFFFFFFFF
            self.lfsr[3] = ~self.lfsr[11] & 0xFFFFFFFF
            self.lfsr[2] = ~self.lfsr[10] & 0xFFFFFFFF
            self.lfsr[1] = ~self.lfsr[9] & 0xFFFFFFFF
            self.lfsr[0] = ~self.lfsr[8] & 0xFFFFFFFF
        ...
        for _ in range(2):
            for j in range(16):
                outfrom_fsm = (self.r1 + self.lfsr[(j+15)%16]) ^ self.r2
                self.lfsr[j] = self._a_mul(self.lfsr[j]) ^ self.lfsr[(j+2)%16] ^ \
                               self._ainv_mul(self.lfsr[(j+11)%16]) ^ outfrom_fsm
                self.lfsr[j] &= 0xFFFFFFFF
                fsmtmp = (self.r2 + self.lfsr[(j+5)%16]) & 0xFFFFFFFF
                self.r2 = self.snow_T0[self._byte_n(0, self.r1)] ^ \
                          self.snow_T1[self._byte_n(1, self.r1)] ^ \
                          self.snow_T2[self._byte_n(2, self.r1)] ^ \
                          self.snow_T3[self._byte_n(3, self.r1)]
                self.r1 = fsmtmp
    ...
    def keystream(self):
        ...
        for i in range(16):
            self.lfsr[i] = self._a_mul(self.lfsr[i]) ^ self.lfsr[(i+2)%16] ^ \
                           self._ainv_mul(self.lfsr[(i+11)%16])
            self.lfsr[i] &= 0xFFFFFFFF
            fsmtmp = (self.r2 + self.lfsr[(i+5)%16]) & 0xFFFFFFFF
            self.r2 = self.snow_T0[self._byte_n(0, self.r1)] ^ \
                      self.snow_T1[self._byte_n(1, self.r1)] ^ \
                      self.snow_T2[self._byte_n(2, self.r1)] ^ \
                      self.snow_T3[self._byte_n(3, self.r1)]
            self.r1 = fsmtmp
            ks[i] = ((self.r1 + self.lfsr[i]) ^ self.r2 ^ self.lfsr[(i+1)%16]) & 0xFFFFFFFF
    ...
    def iv_setup(self, iv):
        ...
        iv0 = (iv[0] << 24) | (iv[1] << 16) | (iv[2] << 8) | iv[3]
        iv1 = (iv[4] << 24) | (iv[5] << 16) | (iv[6] << 8) | iv[7]
        iv2 = (iv[8] << 24) | (iv[9] << 16) | (iv[10] << 8) | iv[11]
        iv3 = (iv[12] << 24) | (iv[13] << 16) | (iv[14] << 8) | iv[15]
        
        self.load_key(iv0, iv1, iv2, iv3)
    
    def xor_key_stream(self, dst, src):
        ...
        n = 0
        src_len = len(src)
        dst = bytearray(dst) if isinstance(dst, bytes) else dst
        ...
        while n < src_len:
            ...
            for i in range(16):
                ...
                if n < src_len:
                    dst[n] = src[n] ^ (word & 0xff)
                    n += 1
                if n < src_len:
                    dst[n] = src[n] ^ ((word >> 8) & 0xff)
                    n += 1
                if n < src_len:
                    dst[n] = src[n] ^ ((word >> 16) & 0xff)
                    n += 1
                if n < src_len:
                    dst[n] = src[n] ^ ((word >> 24) & 0xff)
                    n += 1
        
        return bytes(dst) if isinstance(dst, bytearray) else dst
    
    @classmethod
    def new_cipher(cls, key, iv):
        """Cria uma nova instância com chave e IV"""
        if len(key) not in (16, 32):
            raise ValueError("snow2: invalid key size")
        if len(iv) != 16:
            raise ValueError("snow2: invalid IV size")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SNOW 2.0 - Cifra de fluxo')
    parser.add_argument('--key', required=True, help='Chave em hexadecimal (32 ou 64 caracteres)')
    parser.add_argument('--iv', required=True, help='IV em hexadecimal (32 caracteres)')
    parser.add_argument('--decrypt', action='store_true', help='Modo descriptografia')
    ...
    try:
        ...
        # Validar tamanhos
        if len(key) not in (16, 32):
            sys.stderr.write(f"Erro: Chave deve ter 16 ou 32 bytes (tem {len(key)} bytes)\n")
            sys.exit(1)
        
        if len(iv) != 16:
            sys.stderr.write(f"Erro: IV deve ter 16 bytes (tem {len(iv)} bytes)\n")
            sys.exit(1)
        
        # Criar cipher
        cipher = Snow2Cipher.new_cipher(key, iv)
        
        # Ler toda entrada do stdin
        data = sys.stdin.buffer.read()
        
        # Processar
        output = bytearray(len(data))
        cipher.xor_key_stream(output, data)
