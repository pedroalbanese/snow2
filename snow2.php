...
/**
 * Implementação do SNOW 2.0 em PHP
 * Baseada fielmente no código Go de referência
 */
class Snow2Cipher {
    private $lfsr = [];
    private $r1 = 0;
    ...
    // Propriedades estáticas para as tabelas
    private static $snow_alpha_mul = null;
    private static $snow_alphainv_mul = null;
    private static $snow_T0 = null;
    private static $snow_T1 = null;
    private static $snow_T2 = null;
    private static $snow_T3 = null;
    
    /**
     * Inicialização das tabelas
     */
    private static function initTables() {
        // Verificar se já foram inicializadas
        if (self::$snow_alpha_mul !== null) {
            return;
        }
        
        // Inicializar snow_alpha_mul
        self::$snow_alpha_mul = [
        ...
        // Inicializar snow_alphainv_mul
        self::$snow_alphainv_mul = [
    ...
    public function __construct($key) {
        self::initTables(); // Garantir que as tabelas estão inicializadas
        ...
        if ($keyLen !== 16 && $keyLen !== 32) {
            throw new InvalidArgumentException("Invalid key size: must be 16 or 32 bytes");
        }
        
        $this->key = $key;
        $this->keysize = $keyLen * 8;
    }
    
    /**
     * Extrai o n-ésimo byte de w (0 = LSB, 3 = MSB)
     */
    private function byte_n($n, $w) {
        return ($w >> (8 * $n)) & 0xff;
    }
     ...
     * a_mul do C
     */
    private function a_mul($w) {
        return ($w << 8) ^ self::$snow_alpha_mul[$w >> 24];
    }
    
    /**
     * ainv_mul do C
     */
    private function ainv_mul($w) {
        return ($w >> 8) ^ self::$snow_alphainv_mul[$w & 0xff];
    }
    
    /**
     * Carrega a chave e IV
     */
    public function loadKey($IV0, $IV1, $IV2, $IV3) {
        if ($this->keysize == 128) {
            $this->lfsr[15] = (ord($this->key[0]) << 24) | (ord($this->key[1]) << 16) |
                               (ord($this->key[2]) << 8) | ord($this->key[3]);
            $this->lfsr[14] = (ord($this->key[4]) << 24) | (ord($this->key[5]) << 16) |
                               (ord($this->key[6]) << 8) | ord($this->key[7]);
            $this->lfsr[13] = (ord($this->key[8]) << 24) | (ord($this->key[9]) << 16) |
                               (ord($this->key[10]) << 8) | ord($this->key[11]);
            $this->lfsr[12] = (ord($this->key[12]) << 24) | (ord($this->key[13]) << 16) |
                               (ord($this->key[14]) << 8) | ord($this->key[15]);
            
            $this->lfsr[11] = ~$this->lfsr[15] & 0xFFFFFFFF;
            $this->lfsr[10] = ~$this->lfsr[14] & 0xFFFFFFFF;
            $this->lfsr[9] = ~$this->lfsr[13] & 0xFFFFFFFF;
            $this->lfsr[8] = ~$this->lfsr[12] & 0xFFFFFFFF;
            $this->lfsr[7] = $this->lfsr[15];
            $this->lfsr[6] = $this->lfsr[14];
            $this->lfsr[5] = $this->lfsr[13];
            $this->lfsr[4] = $this->lfsr[12];
            $this->lfsr[3] = ~$this->lfsr[15] & 0xFFFFFFFF;
            $this->lfsr[2] = ~$this->lfsr[14] & 0xFFFFFFFF;
            $this->lfsr[1] = ~$this->lfsr[13] & 0xFFFFFFFF;
            $this->lfsr[0] = ~$this->lfsr[12] & 0xFFFFFFFF;
        } else { // 256 bits
            for ($i = 0; $i < 8; $i++) {
                $this->lfsr[15 - $i] = (ord($this->key[4 * $i]) << 24) |
                                        (ord($this->key[4 * $i + 1]) << 16) |
                                        (ord($this->key[4 * $i + 2]) << 8) |
                                        ord($this->key[4 * $i + 3]);
            }
            
            $this->lfsr[7] = ~$this->lfsr[15] & 0xFFFFFFFF;
            $this->lfsr[6] = ~$this->lfsr[14] & 0xFFFFFFFF;
            $this->lfsr[5] = ~$this->lfsr[13] & 0xFFFFFFFF;
            $this->lfsr[4] = ~$this->lfsr[12] & 0xFFFFFFFF;
            $this->lfsr[3] = ~$this->lfsr[11] & 0xFFFFFFFF;
            $this->lfsr[2] = ~$this->lfsr[10] & 0xFFFFFFFF;
            $this->lfsr[1] = ~$this->lfsr[9] & 0xFFFFFFFF;
            $this->lfsr[0] = ~$this->lfsr[8] & 0xFFFFFFFF;
        }
        ...
        // 32 initial clockings (2 * 16)
        for ($i = 0; $i < 2; $i++) {
            $outfrom_fsm = ($this->r1 + $this->lfsr[15]) ^ $this->r2;
            $this->lfsr[0] = $this->a_mul($this->lfsr[0]) ^ $this->lfsr[2] ^ $this->ainv_mul($this->lfsr[11]) ^ $outfrom_fsm;
            $this->lfsr[0] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[5]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            $this->r1 = $fsmtmp;
            
            $outfrom_fsm = ($this->r1 + $this->lfsr[0]) ^ $this->r2;
            $this->lfsr[1] = $this->a_mul($this->lfsr[1]) ^ $this->lfsr[3] ^ $this->ainv_mul($this->lfsr[12]) ^ $outfrom_fsm;
            $this->lfsr[1] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[6]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[1]) ^ $this->r2;
            $this->lfsr[2] = $this->a_mul($this->lfsr[2]) ^ $this->lfsr[4] ^ $this->ainv_mul($this->lfsr[13]) ^ $outfrom_fsm;
            $this->lfsr[2] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[7]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[2]) ^ $this->r2;
            $this->lfsr[3] = $this->a_mul($this->lfsr[3]) ^ $this->lfsr[5] ^ $this->ainv_mul($this->lfsr[14]) ^ $outfrom_fsm;
            $this->lfsr[3] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[8]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[3]) ^ $this->r2;
            $this->lfsr[4] = $this->a_mul($this->lfsr[4]) ^ $this->lfsr[6] ^ $this->ainv_mul($this->lfsr[15]) ^ $outfrom_fsm;
            $this->lfsr[4] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[9]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[4]) ^ $this->r2;
            $this->lfsr[5] = $this->a_mul($this->lfsr[5]) ^ $this->lfsr[7] ^ $this->ainv_mul($this->lfsr[0]) ^ $outfrom_fsm;
            $this->lfsr[5] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[10]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[5]) ^ $this->r2;
            $this->lfsr[6] = $this->a_mul($this->lfsr[6]) ^ $this->lfsr[8] ^ $this->ainv_mul($this->lfsr[1]) ^ $outfrom_fsm;
            $this->lfsr[6] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[11]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[6]) ^ $this->r2;
            $this->lfsr[7] = $this->a_mul($this->lfsr[7]) ^ $this->lfsr[9] ^ $this->ainv_mul($this->lfsr[2]) ^ $outfrom_fsm;
            $this->lfsr[7] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[12]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[7]) ^ $this->r2;
            $this->lfsr[8] = $this->a_mul($this->lfsr[8]) ^ $this->lfsr[10] ^ $this->ainv_mul($this->lfsr[3]) ^ $outfrom_fsm;
            $this->lfsr[8] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[13]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[8]) ^ $this->r2;
            $this->lfsr[9] = $this->a_mul($this->lfsr[9]) ^ $this->lfsr[11] ^ $this->ainv_mul($this->lfsr[4]) ^ $outfrom_fsm;
            $this->lfsr[9] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[14]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[9]) ^ $this->r2;
            $this->lfsr[10] = $this->a_mul($this->lfsr[10]) ^ $this->lfsr[12] ^ $this->ainv_mul($this->lfsr[5]) ^ $outfrom_fsm;
            $this->lfsr[10] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[15]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[10]) ^ $this->r2;
            $this->lfsr[11] = $this->a_mul($this->lfsr[11]) ^ $this->lfsr[13] ^ $this->ainv_mul($this->lfsr[6]) ^ $outfrom_fsm;
            $this->lfsr[11] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[0]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[11]) ^ $this->r2;
            $this->lfsr[12] = $this->a_mul($this->lfsr[12]) ^ $this->lfsr[14] ^ $this->ainv_mul($this->lfsr[7]) ^ $outfrom_fsm;
            $this->lfsr[12] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[1]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[12]) ^ $this->r2;
            $this->lfsr[13] = $this->a_mul($this->lfsr[13]) ^ $this->lfsr[15] ^ $this->ainv_mul($this->lfsr[8]) ^ $outfrom_fsm;
            $this->lfsr[13] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[2]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[13]) ^ $this->r2;
            $this->lfsr[14] = $this->a_mul($this->lfsr[14]) ^ $this->lfsr[0] ^ $this->ainv_mul($this->lfsr[9]) ^ $outfrom_fsm;
            $this->lfsr[14] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[3]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
            ...
            $outfrom_fsm = ($this->r1 + $this->lfsr[14]) ^ $this->r2;
            $this->lfsr[15] = $this->a_mul($this->lfsr[15]) ^ $this->lfsr[1] ^ $this->ainv_mul($this->lfsr[10]) ^ $outfrom_fsm;
            $this->lfsr[15] &= 0xFFFFFFFF;
            $fsmtmp = ($this->r2 + $this->lfsr[4]) & 0xFFFFFFFF;
            $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                        self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                        self::$snow_T2[$this->byte_n(2, $this->r1)] ^ 
                        self::$snow_T3[$this->byte_n(3, $this->r1)];
    ...
    /**
     * Gera um bloco de keystream (16 palavras de 32 bits)
     */
    public function keystream() {
        $ks = array_fill(0, 16, 0);
        
        $this->lfsr[0] = $this->a_mul($this->lfsr[0]) ^ $this->lfsr[2] ^ $this->ainv_mul($this->lfsr[11]);
        $this->lfsr[0] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[5]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[0] = (($this->r1 + $this->lfsr[0]) ^ $this->r2 ^ $this->lfsr[1]) & 0xFFFFFFFF;
        
        $this->lfsr[1] = $this->a_mul($this->lfsr[1]) ^ $this->lfsr[3] ^ $this->ainv_mul($this->lfsr[12]);
        $this->lfsr[1] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[6]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[1] = (($this->r1 + $this->lfsr[1]) ^ $this->r2 ^ $this->lfsr[2]) & 0xFFFFFFFF;
        
        $this->lfsr[2] = $this->a_mul($this->lfsr[2]) ^ $this->lfsr[4] ^ $this->ainv_mul($this->lfsr[13]);
        $this->lfsr[2] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[7]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[2] = (($this->r1 + $this->lfsr[2]) ^ $this->r2 ^ $this->lfsr[3]) & 0xFFFFFFFF;
        
        $this->lfsr[3] = $this->a_mul($this->lfsr[3]) ^ $this->lfsr[5] ^ $this->ainv_mul($this->lfsr[14]);
        $this->lfsr[3] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[8]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[3] = (($this->r1 + $this->lfsr[3]) ^ $this->r2 ^ $this->lfsr[4]) & 0xFFFFFFFF;
        
        $this->lfsr[4] = $this->a_mul($this->lfsr[4]) ^ $this->lfsr[6] ^ $this->ainv_mul($this->lfsr[15]);
        $this->lfsr[4] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[9]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[4] = (($this->r1 + $this->lfsr[4]) ^ $this->r2 ^ $this->lfsr[5]) & 0xFFFFFFFF;
        
        $this->lfsr[5] = $this->a_mul($this->lfsr[5]) ^ $this->lfsr[7] ^ $this->ainv_mul($this->lfsr[0]);
        $this->lfsr[5] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[10]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[5] = (($this->r1 + $this->lfsr[5]) ^ $this->r2 ^ $this->lfsr[6]) & 0xFFFFFFFF;
        
        $this->lfsr[6] = $this->a_mul($this->lfsr[6]) ^ $this->lfsr[8] ^ $this->ainv_mul($this->lfsr[1]);
        $this->lfsr[6] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[11]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[6] = (($this->r1 + $this->lfsr[6]) ^ $this->r2 ^ $this->lfsr[7]) & 0xFFFFFFFF;
        
        $this->lfsr[7] = $this->a_mul($this->lfsr[7]) ^ $this->lfsr[9] ^ $this->ainv_mul($this->lfsr[2]);
        $this->lfsr[7] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[12]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[7] = (($this->r1 + $this->lfsr[7]) ^ $this->r2 ^ $this->lfsr[8]) & 0xFFFFFFFF;
        
        $this->lfsr[8] = $this->a_mul($this->lfsr[8]) ^ $this->lfsr[10] ^ $this->ainv_mul($this->lfsr[3]);
        $this->lfsr[8] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[13]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[8] = (($this->r1 + $this->lfsr[8]) ^ $this->r2 ^ $this->lfsr[9]) & 0xFFFFFFFF;
        
        $this->lfsr[9] = $this->a_mul($this->lfsr[9]) ^ $this->lfsr[11] ^ $this->ainv_mul($this->lfsr[4]);
        $this->lfsr[9] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[14]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[9] = (($this->r1 + $this->lfsr[9]) ^ $this->r2 ^ $this->lfsr[10]) & 0xFFFFFFFF;
        
        $this->lfsr[10] = $this->a_mul($this->lfsr[10]) ^ $this->lfsr[12] ^ $this->ainv_mul($this->lfsr[5]);
        $this->lfsr[10] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[15]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[10] = (($this->r1 + $this->lfsr[10]) ^ $this->r2 ^ $this->lfsr[11]) & 0xFFFFFFFF;
        
        $this->lfsr[11] = $this->a_mul($this->lfsr[11]) ^ $this->lfsr[13] ^ $this->ainv_mul($this->lfsr[6]);
        $this->lfsr[11] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[0]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[11] = (($this->r1 + $this->lfsr[11]) ^ $this->r2 ^ $this->lfsr[12]) & 0xFFFFFFFF;
        
        $this->lfsr[12] = $this->a_mul($this->lfsr[12]) ^ $this->lfsr[14] ^ $this->ainv_mul($this->lfsr[7]);
        $this->lfsr[12] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[1]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[12] = (($this->r1 + $this->lfsr[12]) ^ $this->r2 ^ $this->lfsr[13]) & 0xFFFFFFFF;
        
        $this->lfsr[13] = $this->a_mul($this->lfsr[13]) ^ $this->lfsr[15] ^ $this->ainv_mul($this->lfsr[8]);
        $this->lfsr[13] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[2]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[13] = (($this->r1 + $this->lfsr[13]) ^ $this->r2 ^ $this->lfsr[14]) & 0xFFFFFFFF;
        
        $this->lfsr[14] = $this->a_mul($this->lfsr[14]) ^ $this->lfsr[0] ^ $this->ainv_mul($this->lfsr[9]);
        $this->lfsr[14] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[3]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[14] = (($this->r1 + $this->lfsr[14]) ^ $this->r2 ^ $this->lfsr[15]) & 0xFFFFFFFF;
        
        $this->lfsr[15] = $this->a_mul($this->lfsr[15]) ^ $this->lfsr[1] ^ $this->ainv_mul($this->lfsr[10]);
        $this->lfsr[15] &= 0xFFFFFFFF;
        $fsmtmp = ($this->r2 + $this->lfsr[4]) & 0xFFFFFFFF;
        $this->r2 = self::$snow_T0[$this->byte_n(0, $this->r1)] ^ 
                    self::$snow_T1[$this->byte_n(1, $this->r1)] ^ 
                    ...
                    self::$snow_T3[$this->byte_n(3, $this->r1)];
        $this->r1 = $fsmtmp;
        $ks[15] = (($this->r1 + $this->lfsr[15]) ^ $this->r2 ^ $this->lfsr[0]) & 0xFFFFFFFF;
    ...
    public function ivSetup($iv) {
        ...
        $iv0 = (ord($iv[0]) << 24) | (ord($iv[1]) << 16) | (ord($iv[2]) << 8) | ord($iv[3]);
        $iv1 = (ord($iv[4]) << 24) | (ord($iv[5]) << 16) | (ord($iv[6]) << 8) | ord($iv[7]);
        $iv2 = (ord($iv[8]) << 24) | (ord($iv[9]) << 16) | (ord($iv[10]) << 8) | ord($iv[11]);
        $iv3 = (ord($iv[12]) << 24) | (ord($iv[13]) << 16) | (ord($iv[14]) << 8) | ord($iv[15]);
        
        $this->loadKey($iv0, $iv1, $iv2, $iv3);
    }
    
    /**
     * Aplica XOR com o keystream (little-endian)
     */
    public function xorKeyStream(&$dst, $src) {
        if (strlen($dst) < strlen($src)) {
            throw new InvalidArgumentException("Destination buffer too small");
        }
        ...
        while ($n < $srcLen) {
            ...
            for ($i = 0; $i < 16 && $n < $srcLen; $i++) {
                $word = $ks[$i];
                
                // Little-endian order (LSB first)
                if ($n < $srcLen) {
                    $dst[$n] = chr(ord($src[$n]) ^ ($word & 0xff));
                    $n++;
                }
                if ($n < $srcLen) {
                    $dst[$n] = chr(ord($src[$n]) ^ (($word >> 8) & 0xff));
                    $n++;
                }
                if ($n < $srcLen) {
                    $dst[$n] = chr(ord($src[$n]) ^ (($word >> 16) & 0xff));
                    $n++;
                }
                if ($n < $srcLen) {
                    $dst[$n] = chr(ord($src[$n]) ^ (($word >> 24) & 0xff));
                    $n++;
                }
            }
        }
    ...
    /**
     * Cria uma nova instância com chave e IV
     */
    public static function newCipher($key, $iv) {
        $keyLen = strlen($key);
        $ivLen = strlen($iv);
        
        if ($keyLen != 16 && $keyLen != 32) {
            throw new InvalidArgumentException("snow2: invalid key size");
        }
        if ($ivLen != 16) {
            throw new InvalidArgumentException("snow2: invalid IV size");
        }
...
class Snow2Test {
    public static function runTests() {
        echo "Running SNOW 2.0 tests...\n\n";
        
        self::testEncryptDecrypt();
        self::testKeyIVDependence();
        self::testMultipleBlocks();
        self::testXORKeyStreamEdgeCases();
        self::testInvalidParameters();
        self::testDeterministic();
        self::testOfficialVectors();
    }
    
    private static function testEncryptDecrypt() {
        echo "Test: Encrypt/Decrypt\n";
        ...
        $plaintext = "Hello, SNOW 2.0! This is a test message for encryption and decryption.";
        
        foreach ($testCases as $tc) {
            try {
                // Encrypt
                $ctx1 = Snow2Cipher::newCipher($tc['key'], $tc['iv']);
                $ciphertext = str_repeat("\0", strlen($plaintext));
                $ctx1->xorKeyStream($ciphertext, $plaintext);
                
                // Decrypt
                $ctx2 = Snow2Cipher::newCipher($tc['key'], $tc['iv']);
                $decrypted = str_repeat("\0", strlen($ciphertext));
                ...
                echo "  ✗ {$tc['name']}: ERROR - " . $e->getMessage() . "\n";
            }
        }
        echo "\n";
    }
    
    private static function testKeyIVDependence() {
        echo "Test: Key/IV Dependence\n";
        
        $key1 = str_repeat("\0", 16);
        $iv1 = str_repeat("\0", 16);
        
        $key2 = str_repeat("\0", 16);
        $key2[0] = "\x01";
        $iv2 = str_repeat("\0", 16);
        
        $ctx1 = new Snow2Cipher($key1);
        $ctx1->ivSetup($iv1);
        $ks1 = $ctx1->keystream();
        
        $ctx2 = new Snow2Cipher($key2);
        $ctx2->ivSetup($iv2);
        $ks2 = $ctx2->keystream();
        ...
        echo $different ? "  ✓ Different keys: OK\n" : "  ✗ Different keys: FAILED\n";
        
        // Test IV different
        $iv2 = str_repeat("\0", 16);
        $iv2[0] = "\x01";
        $ctx3 = new Snow2Cipher($key1);
        $ctx3->ivSetup($iv2);
        $ks3 = $ctx3->keystream();
        ...
        echo $different ? "  ✓ Different IVs: OK\n" : "  ✗ Different IVs: FAILED\n";
        echo "\n";
    }
    
    private static function testMultipleBlocks() {
        echo "Test: Multiple Blocks\n";
        
        $key = str_repeat("\0", 16);
        $iv = str_repeat("\0", 16);
        
        $ctx = new Snow2Cipher($key);
        $ctx->ivSetup($iv);
        ...
        echo $different ? "  ✓ Blocks are different: OK\n" : "  ✗ Blocks are identical: FAILED\n";
        ...
        echo "  Info: Blocks 1 and 2 differ in $changes/16 words\n\n";
    }
    
    private static function testXORKeyStreamEdgeCases() {
        echo "Test: XORKeyStream Edge Cases\n";
        
        $key = str_repeat("\0", 16);
        $iv = str_repeat("\0", 16);
        
        $sizes = [0, 1, 2, 3, 4, 5, 6, 7, 8, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129];
        
        $passed = 0;
        foreach ($sizes as $size) {
            try {
                $ctx = Snow2Cipher::newCipher($key, $iv);
                ...
                $ciphertext = str_repeat("\0", $size);
                $ctx->xorKeyStream($ciphertext, $plaintext);
                
                $ctx2 = Snow2Cipher::newCipher($key, $iv);
                $decrypted = str_repeat("\0", $size);
                $ctx2->xorKeyStream($decrypted, $ciphertext);
                
                if ($decrypted === $plaintext) {
                    echo "  ✓ Size " . self::formatSize($size) . ": OK\n";
                    $passed++;
                } else {
                    echo "  ✗ Size " . self::formatSize($size) . ": FAILED\n";
                }
            } catch (Exception $e) {
                echo "  ✗ Size " . self::formatSize($size) . ": ERROR - " . $e->getMessage() . "\n";
            }
        }
        echo "  Passed $passed/" . count($sizes) . " tests\n\n";
    }
    
    private static function testInvalidParameters() {
        ...
        try {
            $key = str_repeat("\0", 24);
            $iv = str_repeat("\0", 16);
            Snow2Cipher::newCipher($key, $iv);
            echo "  ✗ Invalid key size: FAILED (should throw)\n";
        } catch (InvalidArgumentException $e) {
            echo "  ✓ Invalid key size: OK\n";
            $passed++;
        }
        ...
        try {
            ...
            $iv = str_repeat("\0", 8);
            Snow2Cipher::newCipher($key, $iv);
            echo "  ✗ Invalid IV size: FAILED (should throw)\n";
        } catch (InvalidArgumentException $e) {
            echo "  ✓ Invalid IV size: OK\n";
            $passed++;
        }
        ...
        try {
            ...
            $iv = str_repeat("\0", 32);
            Snow2Cipher::newCipher($key, $iv);
            echo "  ✗ IV size > 16: FAILED (should throw)\n";
        } catch (InvalidArgumentException $e) {
            echo "  ✓ IV size > 16: OK\n";
            $passed++;
        }
    ...
    private static function testDeterministic() {
        echo "Test: Deterministic\n";
        
        $key = "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f";
        $iv = "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f";
        
        $ctx1 = new Snow2Cipher($key);
        $ctx1->ivSetup($iv);
        $ks1 = $ctx1->keystream();
        
        $ctx2 = new Snow2Cipher($key);
        $ctx2->ivSetup($iv);
        $ks2 = $ctx2->keystream();
        ...
        for ($i = 0; $i < 16; $i++) {
            if ($ks1[$i] != $ks2[$i]) {
                $match = false;
                echo "  ✗ Mismatch at word $i: 0x" . dechex($ks1[$i]) . " vs 0x" . dechex($ks2[$i]) . "\n";
            }
        }
        
        echo $match ? "  ✓ All keystreams match: OK\n" : "  ✗ Keystreams differ: FAILED\n";
        echo "\n";
    }
    
    private static function testOfficialVectors() {
        echo "Test: Official Vectors\n";
        ...
        $officialTestVectors = [
            ...
            [
                ...
                'keystream' => [0x8D590AE9, 0xA74A7D05, 0x6DC9CA74, 0xB72D1A45, 0x99B0A083]
            ],
            [
                ...
                'iv' => "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                'keystream' => [0xE00982F5, 0x25F02054, 0x214992D8, 0x706F2B20, 0xDA585E5B]
            ],
            [
                'name' => '128-bit - Key=80...00, IV=(1,2,3,4)',
                'keySize' => 128,
                'key' => "\x80\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                'iv' => "\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x04",
                'keystream' => [0xD6403358, 0xE0354A69, 0x57F43FCE, 0x44B4B13F, 0xF78E24C2]
            ],
            [
                ...
                'key' => str_repeat("\xAA", 16),
                'iv' => "\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x04",
                'keystream' => [0xC355385D, 0xB31D6CBD, 0xF774AF53, 0x66C2E877, 0x4DEADAC7]
            ],
            // 256-bit key tests
            [
                ...
                'iv' => "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                'keystream' => [0x0B5BCCE2, 0x0323E28E, 0x0FC20380, 0x9C66AB73, 0xCA35A680]
            ],
            [
                ...
                'iv' => "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                'keystream' => [0xD9CC22FD, 0x861492D0, 0xAE6F43FB, 0x0F072012, 0x078C5AEE]
            ],
            [
                ...
                'key' => "\x80\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                'iv' => "\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x04",
                'keystream' => [0x7861080D, 0x5755E90B, 0x736F1091, 0x6ED519B1, 0x2C1A3A42]
            ],
            [
                ...
                'key' => str_repeat("\xAA", 32),
                'iv' => "\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x04",
                'keystream' => [0x29261FCE, 0x5ED03820, 0x1D6AFAF8, 0xB87E74FE, 0xD49ECB10]
            ]
        ];
        
        $passed = 0;
        $total = count($officialTestVectors);
        
        foreach ($officialTestVectors as $tv) {
            echo "  Testing: {$tv['name']}\n";
            
            try {
                $ctx = new Snow2Cipher($tv['key']);
                $ctx->ivSetup($tv['iv']);
                
                $ks = $ctx->keystream();
                
                $vectorPassed = true;
                for ($i = 0; $i < count($tv['keystream']); $i++) {
                    if ($ks[$i] != $tv['keystream'][$i]) {
                        echo "    ✗ Word " . ($i+1) . ": got 0x" . str_pad(strtoupper(dechex($ks[$i])), 8, '0', STR_PAD_LEFT) . 
                             ", want 0x" . str_pad(strtoupper(dechex($tv['keystream'][$i])), 8, '0', STR_PAD_LEFT) . "\n";
                        $vectorPassed = false;
                    } else {
                        echo "    ✓ Word " . ($i+1) . ": 0x" . str_pad(strtoupper(dechex($ks[$i])), 8, '0', STR_PAD_LEFT) . "\n";
                    }
                }
