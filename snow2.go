// snow2.go - Implementação fiel ao código C de referência
package snow2
...
// Tabelas pré-computadas do SNOW 2.0
var snow_alpha_mul = [256]uint32{
...
var snow_alphainv_mul = [256]uint32{
...
// byte_n extrai o n-ésimo byte de w (0 = LSB, 3 = MSB)
func byte_n(n int, w uint32) byte {
	return byte((w >> uint(8*n)) & 0xff)
}

// a_mul do C
func a_mul(w uint32) uint32 {
	return (w << 8) ^ snow_alpha_mul[w>>24]
}

// ainv_mul do C
func ainv_mul(w uint32) uint32 {
	return (w >> 8) ^ snow_alphainv_mul[w&0xff]
}

func NewSnowCtx(key []byte) *SnowCtx {
	ctx := &SnowCtx{
...
func (ctx *SnowCtx) LoadKey(IV0, IV1, IV2, IV3 uint32) {
	if ctx.keysize == 128 {
		ctx.lfsr[15] = uint32(ctx.key[0])<<24 | uint32(ctx.key[1])<<16 |
			uint32(ctx.key[2])<<8 | uint32(ctx.key[3])
		ctx.lfsr[14] = uint32(ctx.key[4])<<24 | uint32(ctx.key[5])<<16 |
			uint32(ctx.key[6])<<8 | uint32(ctx.key[7])
		ctx.lfsr[13] = uint32(ctx.key[8])<<24 | uint32(ctx.key[9])<<16 |
			uint32(ctx.key[10])<<8 | uint32(ctx.key[11])
		ctx.lfsr[12] = uint32(ctx.key[12])<<24 | uint32(ctx.key[13])<<16 |
			uint32(ctx.key[14])<<8 | uint32(ctx.key[15])

		ctx.lfsr[11] = ^ctx.lfsr[15]
		ctx.lfsr[10] = ^ctx.lfsr[14]
		ctx.lfsr[9] = ^ctx.lfsr[13]
		ctx.lfsr[8] = ^ctx.lfsr[12]
		ctx.lfsr[7] = ctx.lfsr[15]
		ctx.lfsr[6] = ctx.lfsr[14]
		ctx.lfsr[5] = ctx.lfsr[13]
		ctx.lfsr[4] = ctx.lfsr[12]
		ctx.lfsr[3] = ^ctx.lfsr[15]
		ctx.lfsr[2] = ^ctx.lfsr[14]
		ctx.lfsr[1] = ^ctx.lfsr[13]
		ctx.lfsr[0] = ^ctx.lfsr[12]
	} else { // 256 bits
		for i := 0; i < 8; i++ {
			ctx.lfsr[15-i] = uint32(ctx.key[4*i])<<24 |
				uint32(ctx.key[4*i+1])<<16 |
				uint32(ctx.key[4*i+2])<<8 |
				uint32(ctx.key[4*i+3])
		}
		...
		ctx.lfsr[5] = ^ctx.lfsr[13]
		ctx.lfsr[4] = ^ctx.lfsr[12]
		ctx.lfsr[3] = ^ctx.lfsr[11]
		ctx.lfsr[2] = ^ctx.lfsr[10]
		ctx.lfsr[1] = ^ctx.lfsr[9]
		ctx.lfsr[0] = ^ctx.lfsr[8]
	}
	...
	for i := 0; i < 2; i++ {
		var outfrom_fsm, fsmtmp uint32

		outfrom_fsm = (ctx.r1 + ctx.lfsr[15]) ^ ctx.r2
		ctx.lfsr[0] = a_mul(ctx.lfsr[0]) ^ ctx.lfsr[2] ^ ainv_mul(ctx.lfsr[11]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[5]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[0]) ^ ctx.r2
		ctx.lfsr[1] = a_mul(ctx.lfsr[1]) ^ ctx.lfsr[3] ^ ainv_mul(ctx.lfsr[12]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[6]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[1]) ^ ctx.r2
		ctx.lfsr[2] = a_mul(ctx.lfsr[2]) ^ ctx.lfsr[4] ^ ainv_mul(ctx.lfsr[13]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[7]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[2]) ^ ctx.r2
		ctx.lfsr[3] = a_mul(ctx.lfsr[3]) ^ ctx.lfsr[5] ^ ainv_mul(ctx.lfsr[14]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[8]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[3]) ^ ctx.r2
		ctx.lfsr[4] = a_mul(ctx.lfsr[4]) ^ ctx.lfsr[6] ^ ainv_mul(ctx.lfsr[15]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[9]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[4]) ^ ctx.r2
		ctx.lfsr[5] = a_mul(ctx.lfsr[5]) ^ ctx.lfsr[7] ^ ainv_mul(ctx.lfsr[0]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[10]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[5]) ^ ctx.r2
		ctx.lfsr[6] = a_mul(ctx.lfsr[6]) ^ ctx.lfsr[8] ^ ainv_mul(ctx.lfsr[1]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[11]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[6]) ^ ctx.r2
		ctx.lfsr[7] = a_mul(ctx.lfsr[7]) ^ ctx.lfsr[9] ^ ainv_mul(ctx.lfsr[2]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[12]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[7]) ^ ctx.r2
		ctx.lfsr[8] = a_mul(ctx.lfsr[8]) ^ ctx.lfsr[10] ^ ainv_mul(ctx.lfsr[3]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[13]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[8]) ^ ctx.r2
		ctx.lfsr[9] = a_mul(ctx.lfsr[9]) ^ ctx.lfsr[11] ^ ainv_mul(ctx.lfsr[4]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[14]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[9]) ^ ctx.r2
		ctx.lfsr[10] = a_mul(ctx.lfsr[10]) ^ ctx.lfsr[12] ^ ainv_mul(ctx.lfsr[5]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[15]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[10]) ^ ctx.r2
		ctx.lfsr[11] = a_mul(ctx.lfsr[11]) ^ ctx.lfsr[13] ^ ainv_mul(ctx.lfsr[6]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[0]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[11]) ^ ctx.r2
		ctx.lfsr[12] = a_mul(ctx.lfsr[12]) ^ ctx.lfsr[14] ^ ainv_mul(ctx.lfsr[7]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[1]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[12]) ^ ctx.r2
		ctx.lfsr[13] = a_mul(ctx.lfsr[13]) ^ ctx.lfsr[15] ^ ainv_mul(ctx.lfsr[8]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[2]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[13]) ^ ctx.r2
		ctx.lfsr[14] = a_mul(ctx.lfsr[14]) ^ ctx.lfsr[0] ^ ainv_mul(ctx.lfsr[9]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[3]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp

		outfrom_fsm = (ctx.r1 + ctx.lfsr[14]) ^ ctx.r2
		ctx.lfsr[15] = a_mul(ctx.lfsr[15]) ^ ctx.lfsr[1] ^ ainv_mul(ctx.lfsr[10]) ^ outfrom_fsm
		fsmtmp = ctx.r2 + ctx.lfsr[4]
		ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
		ctx.r1 = fsmtmp
	}
}

func (ctx *SnowCtx) Keystream() [16]uint32 {
	var ks [16]uint32
	var fsmtmp uint32

	ctx.lfsr[0] = a_mul(ctx.lfsr[0]) ^ ctx.lfsr[2] ^ ainv_mul(ctx.lfsr[11])
	fsmtmp = ctx.r2 + ctx.lfsr[5]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[0] = (ctx.r1 + ctx.lfsr[0]) ^ ctx.r2 ^ ctx.lfsr[1]

	ctx.lfsr[1] = a_mul(ctx.lfsr[1]) ^ ctx.lfsr[3] ^ ainv_mul(ctx.lfsr[12])
	fsmtmp = ctx.r2 + ctx.lfsr[6]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[1] = (ctx.r1 + ctx.lfsr[1]) ^ ctx.r2 ^ ctx.lfsr[2]

	ctx.lfsr[2] = a_mul(ctx.lfsr[2]) ^ ctx.lfsr[4] ^ ainv_mul(ctx.lfsr[13])
	fsmtmp = ctx.r2 + ctx.lfsr[7]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[2] = (ctx.r1 + ctx.lfsr[2]) ^ ctx.r2 ^ ctx.lfsr[3]

	ctx.lfsr[3] = a_mul(ctx.lfsr[3]) ^ ctx.lfsr[5] ^ ainv_mul(ctx.lfsr[14])
	fsmtmp = ctx.r2 + ctx.lfsr[8]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[3] = (ctx.r1 + ctx.lfsr[3]) ^ ctx.r2 ^ ctx.lfsr[4]

	ctx.lfsr[4] = a_mul(ctx.lfsr[4]) ^ ctx.lfsr[6] ^ ainv_mul(ctx.lfsr[15])
	fsmtmp = ctx.r2 + ctx.lfsr[9]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[4] = (ctx.r1 + ctx.lfsr[4]) ^ ctx.r2 ^ ctx.lfsr[5]

	ctx.lfsr[5] = a_mul(ctx.lfsr[5]) ^ ctx.lfsr[7] ^ ainv_mul(ctx.lfsr[0])
	fsmtmp = ctx.r2 + ctx.lfsr[10]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[5] = (ctx.r1 + ctx.lfsr[5]) ^ ctx.r2 ^ ctx.lfsr[6]

	ctx.lfsr[6] = a_mul(ctx.lfsr[6]) ^ ctx.lfsr[8] ^ ainv_mul(ctx.lfsr[1])
	fsmtmp = ctx.r2 + ctx.lfsr[11]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[6] = (ctx.r1 + ctx.lfsr[6]) ^ ctx.r2 ^ ctx.lfsr[7]

	ctx.lfsr[7] = a_mul(ctx.lfsr[7]) ^ ctx.lfsr[9] ^ ainv_mul(ctx.lfsr[2])
	fsmtmp = ctx.r2 + ctx.lfsr[12]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[7] = (ctx.r1 + ctx.lfsr[7]) ^ ctx.r2 ^ ctx.lfsr[8]

	ctx.lfsr[8] = a_mul(ctx.lfsr[8]) ^ ctx.lfsr[10] ^ ainv_mul(ctx.lfsr[3])
	fsmtmp = ctx.r2 + ctx.lfsr[13]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[8] = (ctx.r1 + ctx.lfsr[8]) ^ ctx.r2 ^ ctx.lfsr[9]

	ctx.lfsr[9] = a_mul(ctx.lfsr[9]) ^ ctx.lfsr[11] ^ ainv_mul(ctx.lfsr[4])
	fsmtmp = ctx.r2 + ctx.lfsr[14]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[9] = (ctx.r1 + ctx.lfsr[9]) ^ ctx.r2 ^ ctx.lfsr[10]

	ctx.lfsr[10] = a_mul(ctx.lfsr[10]) ^ ctx.lfsr[12] ^ ainv_mul(ctx.lfsr[5])
	fsmtmp = ctx.r2 + ctx.lfsr[15]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[10] = (ctx.r1 + ctx.lfsr[10]) ^ ctx.r2 ^ ctx.lfsr[11]

	ctx.lfsr[11] = a_mul(ctx.lfsr[11]) ^ ctx.lfsr[13] ^ ainv_mul(ctx.lfsr[6])
	fsmtmp = ctx.r2 + ctx.lfsr[0]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[11] = (ctx.r1 + ctx.lfsr[11]) ^ ctx.r2 ^ ctx.lfsr[12]

	ctx.lfsr[12] = a_mul(ctx.lfsr[12]) ^ ctx.lfsr[14] ^ ainv_mul(ctx.lfsr[7])
	fsmtmp = ctx.r2 + ctx.lfsr[1]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[12] = (ctx.r1 + ctx.lfsr[12]) ^ ctx.r2 ^ ctx.lfsr[13]

	ctx.lfsr[13] = a_mul(ctx.lfsr[13]) ^ ctx.lfsr[15] ^ ainv_mul(ctx.lfsr[8])
	fsmtmp = ctx.r2 + ctx.lfsr[2]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[13] = (ctx.r1 + ctx.lfsr[13]) ^ ctx.r2 ^ ctx.lfsr[14]

	ctx.lfsr[14] = a_mul(ctx.lfsr[14]) ^ ctx.lfsr[0] ^ ainv_mul(ctx.lfsr[9])
	fsmtmp = ctx.r2 + ctx.lfsr[3]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[14] = (ctx.r1 + ctx.lfsr[14]) ^ ctx.r2 ^ ctx.lfsr[15]

	ctx.lfsr[15] = a_mul(ctx.lfsr[15]) ^ ctx.lfsr[1] ^ ainv_mul(ctx.lfsr[10])
	fsmtmp = ctx.r2 + ctx.lfsr[4]
	ctx.r2 = snow_T0[byte_n(0, ctx.r1)] ^ snow_T1[byte_n(1, ctx.r1)] ^ snow_T2[byte_n(2, ctx.r1)] ^ snow_T3[byte_n(3, ctx.r1)]
	ctx.r1 = fsmtmp
	ks[15] = (ctx.r1 + ctx.lfsr[15]) ^ ctx.r2 ^ ctx.lfsr[0]

	return ks
}

// Versão alternativa com ordem little-endian
func (ctx *SnowCtx) XORKeyStream(dst, src []byte) {
	if len(dst) < len(src) {
		panic("snow2: destination buffer too small")
	}
	...
	for n < len(src) {
		...
		for i := 0; i < 16 && n < len(src); i++ {
			word := ks[i]

			// Processar na ordem little-endian (LSB primeiro)
			if n < len(src) {
				dst[n] = src[n] ^ byte(word)
				n++
			}
			if n < len(src) {
				dst[n] = src[n] ^ byte(word>>8)
				n++
			}
			if n < len(src) {
				dst[n] = src[n] ^ byte(word>>16)
				n++
			}
			if n < len(src) {
				dst[n] = src[n] ^ byte(word>>24)
				n++
			}
		}
	}
...
func (ctx *SnowCtx) IVSetup(iv []byte) {
	if len(iv) < 16 {
		padded := make([]byte, 16)
		copy(padded, iv)
		iv = padded
	...
	iv0 := uint32(iv[0])<<24 | uint32(iv[1])<<16 | uint32(iv[2])<<8 | uint32(iv[3])
	iv1 := uint32(iv[4])<<24 | uint32(iv[5])<<16 | uint32(iv[6])<<8 | uint32(iv[7])
	iv2 := uint32(iv[8])<<24 | uint32(iv[9])<<16 | uint32(iv[10])<<8 | uint32(iv[11])
	iv3 := uint32(iv[12])<<24 | uint32(iv[13])<<16 | uint32(iv[14])<<8 | uint32(iv[15])

	ctx.LoadKey(iv0, iv1, iv2, iv3)
}

func NewCipher(key, iv []byte) (*SnowCtx, error) {
	if len(key) != 16 && len(key) != 32 {
		return nil, &KeySizeError{}
	}
	if len(iv) != 16 {
		return nil, &IVSizeError{}
	}
...
func (e *KeySizeError) Error() string { return "snow2: invalid key size" }

type IVSizeError struct{}

func (e *IVSizeError) Error() string { return "snow2: invalid IV size" }
